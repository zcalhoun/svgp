...
def load_dataset(paths):
    """
    This is a temporary alternative function to add in the MODIS data, so we can test
    whether the T_S and EVI data is more useful than the PC1 data.
    """
    modis_data = pd.read_csv(
        "/work/zdc6/weather_underground/durham/stations_w_modis.csv"
    )
    dfs = []
    for p in paths:
        stationId = re.search(r"station=([^/]+)", p).group(1)
        df = pd.read_parquet(p)
        df["t_s_day"] = modis_data.loc[stationId].day_t
        df["t_s_night"] = modis_data.loc[stationId].night_t
        df["evi"] = modis_data.loc[stationId].evi
        dfs.append(df)
